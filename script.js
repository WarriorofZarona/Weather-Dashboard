$(document).ready(function () {
    ...
    $("button").click(function () {
        ...
        if ($.inArray(city, cityHistory) === -1 && city !== "") {
            cityHistory.push(city); // Ensures that blank entries and duplicates do not go on the history list
        };
        if (cityHistory.length > 8) {
            cityHistory.shift(); // removes the first element from an array to ensure 8 items
        };
        $(".history").empty();
        historyList();
        displayWeather();
    ...
    function displayWeather() {
        ...
        $.ajax({ // 5-Day forcast API
            ...
            $(".card-deck").empty();
            $(".forecast").show();
            $.each(fiveDayArr, function (i, fiveDays) { // Creates cards out of 5 day forecast array
